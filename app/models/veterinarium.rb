class Veterinarium < ActiveRecord::Base
	validates :nombre,
            uniqueness: {message: "el nombre ya fue usado"},
	          ...
	          :length => {minimum:2, maximum:30 ,message:"tiene que ser mayor a 1 letra y menor a 20",:allow_blank => TRUE }
  validates :direccion,
    	      :presence  => { :message => ": es necesario una direccion" },
            :length => {minimum:2, maximum:50 ,message:"tiene que ser mayor a 1 letra y menor a 30",:allow_blank => TRUE }
  validates :telefono,
